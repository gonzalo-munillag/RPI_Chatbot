#!/bin/bash
# =============================================================================
# Train & Deploy "Hey Prometheus" Wake Word Model
# =============================================================================
#
# This script:
#   1. Verifies recorded audio samples exist
#   2. Trains a custom wake word model using openWakeWord
#   3. Deploys the model to the wakeword Docker container
#   4. Starts the wake word listener
#
# Prerequisites:
#   - Run record_samples.sh first to record audio samples
...
# Training directories (relative to where script is run)
TRAINING_DIR="$WORKING_DIR/prometheus_training"
...
# Docker paths (relative to working directory, or use parent if running from wakeword subdir)
if [ -f "$WORKING_DIR/docker-compose.yml" ]; then
    DOCKER_COMPOSE_DIR="$WORKING_DIR"
elif [ -f "$WORKING_DIR/../docker-compose.yml" ]; then
    DOCKER_COMPOSE_DIR="$(cd "$WORKING_DIR/.." && pwd)"
else
    DOCKER_COMPOSE_DIR="/var/www/ollama_chatbot"  # Fallback
fi
...
echo "Installing openwakeword with training support..."
# Use --break-system-packages for newer Raspberry Pi OS (Bookworm+)
# Install with [training] extra for the train_custom_model function
pip3 install --quiet --break-system-packages "openwakeword[training]" 2>/dev/null || \
pip3 install --break-system-packages "openwakeword[training]" || \
